 ...
 *	Pi's 8-Relay High Voltage board.
...
const CliCmdType CMD_HELP =
	{
		...
		"\tUsage:      8relayhv -h    Display command options list\n",
		"\tUsage:      8relayhv -h <param>   Display help for <param> command option\n",
		"\tExample:    8relayhv -h write    Display help for \"write\" command option\n"};
...
const CliCmdType CMD_VERSION =
{
	...
	"\tUsage:          8relayhv -v\n",
	"",
	"\tExample:        8relayhv -v  Display the version number\n"};
...
const CliCmdType CMD_WAR =
{
	...
	"\tUsage:          8relayhv -warranty\n",
	"",
	"\tExample:        8relayhv -warranty  Display the warranty text\n"};
...
const CliCmdType CMD_LIST =
	{
		...
		1,
		&doList,
		"\t-list:       List all 8relayhv boards connected,\n\treturn       nr of boards and stack level for every board\n",
		"\tUsage:       8relayhv -list\n",
		"",
		"\tExample:     8relayhv -list display: 1,0 \n"};
...
const CliCmdType CMD_WRITE =
{
	...
	"\tUsage:       8relayhv <id> write <channel> <on/off>\n",
	"\tUsage:       8relayhv <id> write <value>\n",
	"\tExample:     8relayhv 0 write 2 On; Set Relay #2 on Board #0 On\n"};
...
const CliCmdType CMD_READ =
{
	...
	"\tUsage:       8relayhv <id> read <channel>\n",
	"\tUsage:       8relayhv <id> read\n",
	"\tExample:     8relayhv 0 read 2; Read Status of Relay #2 on Board #0\n"};
...
const CliCmdType CMD_TEST =
{
	...
	"",
	"\tUsage:       8relayhv <id> test\n",
	"\tExample:     8relayhv 0 test\n"};

CliCmdType gCmdArray[CMD_ARRAY_SIZE];

char *usage = "Usage:	 8relayhv -h <command>\n"
	"         8relayhv -v\n"
	"         8relayhv -warranty\n"
	"         8relayhv -list\n"
	"         8relayhv <id> write <channel> <on/off>\n"
	"         8relayhv <id> write <value>\n"
	"         8relayhv <id> read <channel>\n"
	"         8relayhv <id> read\n"
	"         8relayhv <id> test\n"
	"Where: <id> = Board level id = 0..7\n"
	"Type 8relayhv -h <command> for more help"; // No trailing newline needed here.
...
int doBoardInit(int stack)
{
	...
	addOffset = (stack & 0x02) + (0x01 & (stack >> 2)) + (0x04 & (stack << 2));
	addOffset ^= 0x07;
	add = RELAY8_HW_I2C_BASE_ADD + addOffset;
	dev = i2cSetup(add);
	if (dev == -1)
	{
		return ERROR;
	...
	{
		add = RELAY8_HW_I2C_ALTERNATE_BASE_ADD + addOffset; //(stack + RELAY8_HW_I2C_ALTERNATE_BASE_ADD) ^ 0x07;
		dev = i2cSetup(add);
		if (dev == -1)
		{
			return ERROR;
		...
		if (ERROR == i2cMem8Read(dev, RELAY8_CFG_REG_ADD, buff, 1))
		{
			printf("8relayhv board id %d not detected\n", stack);
			return ERROR;
		}
	}
...
static void doRelayWrite(int argc, char *argv[])
{
	...
	if ( (argc != 5) && (argc != 4))
	{
		printf("Usage: 8relayhv <id> write <relay number> <on/off> \n");
		printf("Usage: 8relayhv <id> write <relay reg value> \n");
		exit(1);
	}
...
static void doVersion(int argc, char *argv[])
{
	UNUSED(argc);
	UNUSED(argv);
	printf("8relayhv v%d.%d.%d Copyright (c) 2016 - 2020 Sequent Microsystems\n",
	VERSION_BASE, VERSION_MAJOR, VERSION_MINOR);
	printf("\nThis is free software with ABSOLUTELY NO WARRANTY.\n");
	printf("For details type: 8relayhv -warranty\n");
...
static void doList(int argc, char *argv[])
{
	...
	for (i = 0; i < 8; i++)
	{
		jumpers = (i & 0x02) + (0x01 & (i >> 2)) + (0x04 & (i << 2));
		if (boardCheck(RELAY8_HW_I2C_BASE_ADD + jumpers) == OK)
		{
			ids[cnt] = i;
			cnt++;
		}
		...
		{
			if (boardCheck(RELAY8_HW_I2C_ALTERNATE_BASE_ADD + jumpers) == OK)
			{
				ids[cnt] = i;
				cnt++;
			}
