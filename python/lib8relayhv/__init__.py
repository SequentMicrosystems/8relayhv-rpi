...
relayMaskRemap = [0x40, 0x10, 0x20, 0x08, 0x80, 0x04, 0x02, 0x01]
relayChRemap = [6, 4, 5, 3, 7, 2, 1, 0]


def __stackToAdd(stack):
    if stack < 0 or stack > 7:
        raise ValueError('Invalid stack level!')
    add = (stack & 0x02) + (0x01 & (stack >> 2)) + (0x04 & (stack << 2))
    add = add ^ 0x07
    return add
